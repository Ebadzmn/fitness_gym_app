...
class DailyTrackingRequestModel {
  ...
  Map<String, dynamic> toJson() {
    ...
    return {
      ...
      "sick": entity.isSick,
      "water": entity.vital.waterText, // Assuming "1.2 (Lit)" format is acceptable or needs cleaning? Based on prompt "2L"
      ...
      "training": {
        ...
        "trainingPlan": entity.training.plans.map((e) => e.toUpperCase().replaceAll(' ', '_')).toList(),
        "cardioCompleted": entity.training.cardioCompleted,
        "cardioType": entity.training.cardioType.toUpperCase().replaceAll(' ', '_'),
        "duration": entity.training.duration // "30"
      },

      "activityStep": 0, // Not found in entity, defaulting to 0 or need to add field? Assuming 0 for now as per prompt example structure mismatch

      "nutrition": {
        "calories": double.tryParse(_cleanNum(entity.nutrition.caloriesText)) ?? 0,
        "carbs": double.tryParse(_cleanNum(entity.nutrition.carbsText)) ?? 0,
        "protein": double.tryParse(_cleanNum(entity.nutrition.proteinText)) ?? 0,
      ...
      "woman": {
        "cyclePhase": entity.women.cyclePhase ?? "OVULATION", // Default or nullable handling
        "cycleDay": entity.women.cycleDayLabel, // "Monday" vs "14" - Mapping issue? Assuming entity has correct data or string
      ...
      "ped": {
        "dailyDosage": entity.pedHealth.dosageTaken ? "Taken" : "Not Taken", // Logic adjustment needed based on prompt "500mg"
